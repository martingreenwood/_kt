<div class="content">
	...
	if(cached_and_valid(get_stylesheet_directory() . '/cache/businesses.txt')){
		$event_data = file_get_contents(get_stylesheet_directory() . '/cache/businesses.txt');
		$event_data_obj = json_decode($event_data);
	} else {
		$event_data = get_data('http://www.exploresouthlakeland.co.uk/exports/businesses/?i='.KTKEY);
		file_put_contents(get_stylesheet_directory() . '/cache/businesses.txt', $event_data);
		$event_data_obj = json_decode($event_data);
    }
